...
// Default minimum fee per side — J$100 in cents (mirrors the migration default).
const DEFAULT_MIN_FEE_CENTS = 10000;

/**
 * The active minimum-fee floor (guardrail on low-value jobs). Amounts are JMD
 * cents. Applied at quote-accept time — see routes/quotes.js. Safe fallbacks if
 * the config row is missing.
 */
async function getMinFees() {
  const r = await db.query(
    'SELECT min_fee_enabled, min_client_fee, min_provider_fee FROM public.platform_fee_config WHERE id = 1'
  );
  const row = r.rows[0] || {};
  ...
  return {
    enabled: row.min_fee_enabled !== false, // default true
    minClientFee: int(row.min_client_fee, DEFAULT_MIN_FEE_CENTS),
    minProviderFee: int(row.min_provider_fee, DEFAULT_MIN_FEE_CENTS),
  };
}
...
module.exports = { jamaicaToday, getActiveRates, getMinFees, activateDueFees, DEFAULT_CLIENT_RATE, DEFAULT_PROVIDER_RATE, DEFAULT_MIN_FEE_CENTS };
