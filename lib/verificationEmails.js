...
/**
 * New provider submitted for verification — sent to all admin users.
 * @param {string}   providerName    Full name of the provider
 * @param {string}   providerService Primary service category (may be null)
 * @param {string[]} adminEmails     Array of admin email addresses
 */
const sendNewProviderSubmittedEmail = async (providerName, providerService, adminEmails) => {
  if (!adminEmails || adminEmails.length === 0) return;

  const reviewUrl = `${FRONTEND_URL}/admin/verification`;
  const serviceNote = providerService
    ? `<p style="margin:0 0 20px;font-size:14px;color:rgba(15,26,14,0.55);line-height:1.7;">
         <strong>Service:</strong> ${escapeHtml(providerService)}
       </p>`
    : '';
  ...
  await resend.emails.send({
    from: `TendrIt <${process.env.RESEND_FROM_EMAIL}>`,
    to: adminEmails,
    subject: `New verification request — ${escapeHtml(providerName)}`,
    html: shell(
      'New provider verification request',
      `
        <p style="margin:0 0 20px;font-size:15px;color:rgba(15,26,14,0.55);line-height:1.7;">
          <strong>${escapeHtml(providerName)}</strong> has submitted their provider profile for verification
          and is now in the pending queue.
        </p>
        ${serviceNote}
        <div style="margin:0 0 24px;padding:16px;background:#f1f6f1;border-radius:12px;font-size:14px;color:rgba(15,26,14,0.7);line-height:1.6;">
          Please review their documents within <strong>48 hours</strong> to meet the SLA.
        </div>
        ${ctaButton(reviewUrl, 'Review Verification Queue')}
        <p style="margin:24px 0 0;font-size:13px;color:rgba(15,26,14,0.35);line-height:1.6;">
          You are receiving this because you are an administrator on TendrIt.
        </p>
      `
    ),
  });
...
module.exports = { sendProviderApprovedEmail, sendProviderRejectedEmail, sendNewProviderSubmittedEmail };
