...
router.get('/', async (req, res) => {
  try {
    const r = await db.query(
      'SELECT client_rate, provider_rate, client_effective, provider_effective FROM public.platform_fee_config WHERE id = 1'
    );
    ...
    res.status(200).json({ success: true, clientRate: 9.5, providerRate: 12 });
  }
});
